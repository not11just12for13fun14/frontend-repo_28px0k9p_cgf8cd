    ...
    <aside className="w-full lg:w-80 flex-shrink-0">
      <div className="sticky top-20 space-y-6">
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <h3 className="text-base font-semibold text-slate-800">COVID-secure info</h3>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <h3 className="text-base font-semibold text-slate-800">Testimonials</h3>
          <blockquote className="mt-3 text-sm leading-6 text-slate-700 min-h-[84px]">
            “{testimonials[index]}”
          </blockquote>
          <div className="mt-3 flex gap-1">
            {testimonials.map((_, i) => (
              <span
                key={i}
                className={`h-2 w-2 rounded-full ${i === index ? 'bg-rose-500' : 'bg-rose-200'}`}
                aria-hidden
              />
            ))}
          </div>
